      ...
      <div class="flex column gap20">
        ...
        <div v-for="orgData in report.distributionsDataByOrgs" :key="orgData.organizationId" class="flex column gap10">
          ...
          <div class="table-like-list flex-1">
            ...
            <div
              v-for="release in orgData.items" :key="release.releaseId"
              class="table-item-preview gap10 flex align-center space-between"
            >
              <p class="flex-1">{{release.activity?.distributedAt? pretyDate(release.activity?.distributedAt) : '-'}}</p>
              <p class="flex-2">{{release.subject}}</p>
              <p class="flex-1">{{$t(`distribute.origins.${release.origin}`)}}</p>
              <p class="flex-1">{{vOrX(release.activity?.openedNewsAt)}}</p>
              <p class="flex-1">{{vOrX(release.activity?.openedLandingPageAt)}}</p>
              <p class="flex-1">{{release.activity?.openLandingPageCount || (release.activity?.openedLandingPageAt? 1 : '-')}}</p>
              <p class="flex-1">{{vOrX(release.activity?.unsubscribedAt)}}</p>
            </div>
          </div>
        </div>
      </div>
