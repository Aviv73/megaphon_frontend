  ...
  methods: {
    getVal(field) {
      ...
      switch (field) {
        ...
        case 'company':
          return (contact.company?.map(c => this.allCompanies.find(comp => comp._id === c)?.name || '').map(c => c.trim()).filter(Boolean) || []).join(', ') || '-';
        case 'tags':
          return (contact.tags?.map(c => this.allTags.find(tag => tag._id === c)?.name || '').map(c => c.trim()).filter(Boolean) || []).join(', ') || '-';
        case 'unsubscribed':
